...
def get_decoder_results_with_retry(mcp: McpClient, analyzer_id: str,
                                   ...
                                   ann_class: Optional[int] = None) -> list:
    """Get decoder results, retrying until data appears or timeout.

    Args:
        ann_class: If given, only request annotations of this class (server
            filters by row/class, so a later row's annotations are not starved
            by the max_count cutoff of the first row).
    """
