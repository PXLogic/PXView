    ...
    , _(new QRibbonPrivate(this))
{
    ...
    styleActionGroup->addAction(_styleMenu->addAction("蓝色", [&]()
    {
        setColor("rgb(43, 87, 154)");
    }));
    styleActionGroup->addAction(_styleMenu->addAction("绿色", [&]()
    {
        setColor("rgb(33,115,70)");
    }));
    styleActionGroup->addAction(_styleMenu->addAction("红色", [&]()
    {
...
void QRibbon::reinitialize(QMainWindow *window,int index,QString data)
{
    //_->_mainWindow = window;
    ...
    // {
    //     auto menu = i->menu();
    //     ui->tabWidgetMenuBar->setTabText(j, i->text());
    //     ui->tabWidgetMenuBar->setTabText(j, "i->text()");
    //     j++;
    ...
    ui->tabWidgetMenuBar->setTabText(index, data);
}
...
void QRibbon::mousePressEvent(QMouseEvent *evt)
{
    ...
    // if (evt->button() == Qt::LeftButton && evt->y() <  10)
    // {
    //     _->_pressed = true;
