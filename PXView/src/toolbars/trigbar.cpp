    ...
    _setting_button(this)
{
    ...
    _display_menu->addMenu(_language);
    ...
    _pwm_button.setToolButtonStyle(Qt::ToolButtonTextUnderIcon);
    ...
    connect(&_pwm_button, SIGNAL(clicked()),this, SLOT(pwm_clicked()));
...
{
     _pwm_button.setText(L_S(STR_PAGE_TOOLBAR, S_ID(IDS_TOOLBAR_PWM), "PWM"));
    ...
    _setting_button.setText(L_S(STR_PAGE_TOOLBAR, S_ID(IDS_TOOLBAR_DISPLAY), "Settings"));    
...
void TrigBar::reStyle()
{
    ...
    _pwm_button.setIcon(QIcon(iconPath+"/pwm.svg"));
...
void TrigBar::pwm_clicked()
{
    if (_pwm_button.isVisible() && _pwm_button.isEnabled())
    {
        DockOptions *opt = getDockOptions();
        opt->pwmDock = !opt->pwmDock;
        sig_trigger(opt->pwmDock);
        AppConfig::Instance().SaveFrame();
...
{ 
    ...
    bool bPwm = _pwm_action->isVisible() && opt->pwmDock;
...
void TrigBar::on_actionEn_triggered()
{
...
void TrigBar::on_actionCn_triggered()
{
    _language->setIcon(QIcon::fromTheme("file",
...
void TrigBar::on_actionLissajous_triggered()
{
 ...
 void TrigBar::update_view_status()
 {
    ...
    _pwm_button.setEnabled(bEnable);
