...
// P1: computed on the GUI thread so the render worker can rasterize this
// analog channel off-thread. Mirrors the param computation in paint_mid
// above; keep the two in sync. Non-const because it reads GUI-thread state
// (zero ratio / hw offset / samplerate) and _float_scale.
bool AnalogSignal::prepare_raster(const PaintContext &ctx, int left, int right,
  ...
  const int height = get_totalHeight();
  out.top = get_y() - height * 0.5f;
  out.bottom = get_y() + height * 0.5f;
  // zeroY 由 get_zero_ratio() 计算；float 数据路径已在 get_zero_ratio() 中
  // 处理 _zero_offset 超界情况 (返回中心 0.5)。
  out.zeroY = ratio2pos(get_zero_ratio());
  const int width = right - left + 1;
  ...
  out.order = _data->get_ch_order(get_index());
  if (out.order == -1)
    return false;
  ...
  // float 电压数据路径：根据数据实际 min/max 动态计算 float_scale，
  // 使波形适配显示区域（与 paint_mid 相同；paint_mid 会同时回写成员）。
  out.float_scale = _float_scale;
  if (_data->is_float() && _data->has_float_range()) {
    float fmin = 0.0f, fmax = 0.0f;
    _data->get_float_min_max(fmin, fmax);
    const float max_abs = std::max(fabsf(fmin), fabsf(fmax));
    if (max_abs > 1e-6f) {
      out.float_scale = (height * 0.5f) / max_abs;
    }
  }
  ...
  const double index_offset = pixels_offset * samples_per_pixel;
  out.start_index =
      (uint64_t)(ring_start + floor(index_offset)) % cur_sample_count;

  out.show_length = min(floor(cur_sample_count - floor(index_offset)),
                        ceil(width * samples_per_pixel + 1));
  if (out.show_length <= 0)
    return false;
