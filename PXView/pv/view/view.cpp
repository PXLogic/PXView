...
View::View(SigSession *session, pv::toolbars::SamplingBar *sampling_bar,
  ...
  // call that forwards through the inline facades (e.g. headerWidth() 鈫?
  // get_traces() 鈫? get_derived->_own_decode_traces() 鈫? sync_derived_traces()).
  ...
  // through the inline facades (e.g. signals_changed 鈫? _signal_sync).
...
// P1-A: Delayed view-update coalescing timer 鈥? merges bursts of
...
connect(_delayed_view_update_timer, &QTimer::timeout, this, [this]() {
  if (_delayed_view_update_pending) {
    // A full update was requested (e.g. zoom/scroll/resize/decode-done) 鈥?
    ...
    // P2: decode-growth repaint 鈥? decode trace layer only, no signal-pixmap
...
// in cores) 鈥? high util alongside a large EVENT_LAG_MAX means the decode
...
void View::set_data_source(pv::data::DataSource *source) {
  ...
  // data-source binding (idempotent 鈥? only adds cursors that are missing).
...
// 通道重排动画的每帧刷新入口（Trace::on_visual_v_offset_changed 调用）。
//
// 为什么不能用裸 update()：SignalPixmapPass 对信号 pixmap 有缓存，重建条件
// 包含 viewport 的 need_update 标志（见 render_pass.cpp 的 rebuild 判定）。
// 动画期间 scale/offset/signalHeight/vOffset 都没变，若不置 need_update，
// 该 pass 会走"只 blit 旧缓存"的快路径 —— 波形停在原位不动，动画看起来卡死。
// 所以这里必须 set_all_update(true) 强制重建，再让 viewport 与 header 重绘。
//
// header 画的是通道卡片与标签，且自身不做 pixmap 缓存，无条件 update() 即可。
...
void View::set_trig_pos(int percent) {
  auto *src = document_snapshot_source(); // 鍙负 null锛堝鏉ラ噰闆嗭級
  if (!src)
    return;
...
#ifdef PXVIEW_DECODE_PERF
  // P3-D5: timing candidate for the EVENT_LAG_MAX block 鈥? full signal
  // relayout (normalize + group + layout_time_signals).
  const auto _op_t0 = std::chrono::steady_clock::now();
#endif

  // 闃插尽鎬ф竻鐞?:妯″紡鍒囨崲/鍔犺浇鏂囦欢/閲囬泦缁撴潫绛夎矾寰勪細閲嶅缓 _own_signals,
  // 鏃? Signal/Trace 瀵硅薄琚攢姣佸悗, Header/Viewport 缂撳瓨鐨勬嫋鎷?/鎮仠/鍙抽敭
  // 涓婁笅鏂囪８鎸囬拡浼氭偓鍨?,鎵撳紑鐨勬瘺鍒烘护娉㈡诞绐椾篃鍙兘寮曠敤鏃? LogicSignal銆?
  // 娉ㄦ剰:閫氶亾楂樺害鎷栨嫿(浠? Header 鍛戒腑)鏈熼棿 mouseMove 涔熶細璋冪敤鏈嚱鏁板仛澧為噺
  // 閲嶆帓,姝ゆ椂蹇呴』淇濈暀娲诲姩鎷栨嫿鐘舵�?,鏁呯敤 Header::mouse_is_down() 瀹堝崼銆?
...
bool View::is_logic_rendering_mode() const {
  ...
  // Core 灞? SigSession 宸插皢 LOGIC 涓? MSO 鍚堝苟澶勭悊锛坰igsession.cpp:1605/1612锛夛紝
  // View 灞傛墍鏈夊師 `get_work_mode() == LOGIC` 鐨勬覆鏌?/浜や簰鍒嗘敮搴旂粺涓�鏀圭敤鏈皳璇嶏紝
  // 璁? MSO 妯″紡缁ф壙 LOGIC 鐨勫叏閮ㄨ涓猴紙涓婚鑹叉澘娉ㄥ叆銆佹护娉㈡诞绐椼�佷俊鍙峰垎缁勭瓑锛夈�?
...
void View::request_delayed_update() {
  ...
  // serviced when it fires 鈥? no need to restart it.  This naturally
...
void View::clear() {
  ...
  // 璁惧鍒囨崲鏃╂湡锛圕urrentDeviceChangePrev 闃舵锛塤dev_handle 鍙兘涓? nullptr锛?
  // 姝ゆ椂 work_mode 鏌ヨ浼氬け璐ャ�傜敤 document 閰嶇疆鎴栭粯璁ゅ�硷紙闈? DSO锛夐伩鍏嶈鍛婂埛灞忋�?
  int mode = LOGIC;
  if (_data_sync->document_ptr() && _data_sync->document_ptr()->has_signal_config()) {
