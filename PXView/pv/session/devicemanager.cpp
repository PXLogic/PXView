...
ds_device_handle DeviceManager::register_file_device(struct sr_dev_inst *sdi)
{
    ...
    const int scanned_count = static_cast<int>(_scanned_sdi.size());
    ...
    pxv_info("DeviceManager: registered file device sdi=%p handle=%llu",
             reinterpret_cast<void*>(sdi), (unsigned long long)handle);
    return handle;
}
    ...
    ds_device_handle handle) const
{
    if (handle == NULL_HANDLE)
        return nullptr;
    const int idx = static_cast<int>(handle) - 1;
    const int scanned_count = static_cast<int>(_scanned_sdi.size());
    ...
    if (file_idx >= 0 && file_idx < static_cast<int>(_file_sdi.size()))
