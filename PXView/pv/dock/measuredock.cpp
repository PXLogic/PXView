...
public:
  ...
  int heightForWidth(int w) const override { return QLabel::heightForWidth(w); }
};
...
MeasureDock::MeasureDock(QWidget *parent, View *view, SigSession *session)
  ...
  _panel_alpha_note->setSizePolicy(QSizePolicy::Preferred,
                                   QSizePolicy::Minimum);
  ...
  _add_dec_label =
      new QLabel(L_S(STR_PAGE_DLG, S_ID(IDS_DLG_TIME_SAMPLES), "Time/Samples"),
                 _dist_section);
  ...
  connect(_dist_add_btn, &QAbstractButton::clicked, this,
          &MeasureDock::add_dist_measure);
  connect(_edge_add_btn, &QAbstractButton::clicked, this,
          &MeasureDock::add_edge_measure);
  connect(_fen_checkBox, &QCheckBox::checkStateChanged, _view,
          &view::View::set_measure_en);
  connect(_view, &view::View::measure_updated, this,
          &MeasureDock::measure_updated);
...
MeasureDock::~MeasureDock() { REMOVE_UI(this); }

void MeasureDock::set_view(view::View *view) {
  if (_view) {
    disconnect(_view, &view::View::cursor_update, this,
               &MeasureDock::cursor_update);
    disconnect(_view, &view::View::cursor_moving, this,
               &MeasureDock::cursor_moving);
    disconnect(_view, &view::View::cursor_moved, this, &MeasureDock::reCalc);
    disconnect(_view, &view::View::measure_updated, this,
               &MeasureDock::measure_updated);
    disconnect(_fen_checkBox, &QCheckBox::checkStateChanged, _view,
               &view::View::set_measure_en);
  }
  ...
  if (_view) {
    connect(_view, &view::View::cursor_update, this,
            &MeasureDock::cursor_update);
    connect(_view, &view::View::cursor_moving, this,
            &MeasureDock::cursor_moving);
    connect(_view, &view::View::cursor_moved, this, &MeasureDock::reCalc);
    connect(_view, &view::View::measure_updated, this,
            &MeasureDock::measure_updated);
...
void MeasureDock::bind_context(TabContext *ctx) {
  ...
  reload();
  if (ctx && ctx->view()) {
    _fen_checkBox->setChecked(
        ctx->view()->dock_ui_state().dock_measure_fen_enabled);

    auto &dist_rows = ctx->view()->dock_ui_state().dock_measure_dist_rows;
...
void MeasureDock::unbind_context() {
  if (_context && _context->view() && _session && _signals->device() &&
    ...
    _context->view()->dock_ui_state().dock_measure_dist_rows = dist_rows;
    _context->view()->dock_ui_state().dock_measure_edge_rows = edge_rows;
  }
  set_view(nullptr);
  _context = nullptr;
...
void MeasureDock::retranslateUi() {
  _mouse_title->setText(
      L_S(STR_PAGE_DLG, S_ID(IDS_DLG_MOUSE_MEASUREMENT), "Mouse measurement"));
  _fen_checkBox->setText(L_S(STR_PAGE_DLG,
                             S_ID(IDS_DLG_ENABLE_FLOATING_MEASUREMENT),
                             "Enable floating measurement"));
  _panel_alpha_note->setText(
      L_S(STR_PAGE_DLG, S_ID(IDS_DLG_PANEL_ALPHA_NOTE),
          "*Adjust the alpha channel of \"Panel Background\" in Settings > "
          "Style > Global Colors to change panel transparency"));
  _dist_title->setText(
      L_S(STR_PAGE_DLG, S_ID(IDS_DLG_CURSOR_DISTANCE), "Cursor Distance"));
  ...
  _edge_label->setText(
      L_S(STR_PAGE_DLG, S_ID(IDS_DLG_RIS_OR_FAL_EDGE), "Rising/Falling/Edges"));
  _time_label->setText(
      L_S(STR_PAGE_DLG, S_ID(IDS_DLG_TIME_SAMPLES), "Time/Samples"));
...
void MeasureDock::reStyle() {
  ...
  for (auto it = mode_rows->_dist_row_list.begin();
       it != mode_rows->_dist_row_list.end(); it++) {
    (*it).del_bt->setIcon(IconCache::Instance().icon(iconPath + "/del.svg"));
  }

  for (auto it = mode_rows->_edge_row_list.begin();
       it != mode_rows->_edge_row_list.end(); it++) {
    (*it).del_bt->setIcon(IconCache::Instance().icon(iconPath + "/del.svg"));
  }

  for (auto it = mode_rows->_opt_row_list.begin();
       it != mode_rows->_opt_row_list.end(); it++) {
    (*it).del_bt->setIcon(IconCache::Instance().icon(iconPath + "/del.svg"));
  }
...
void MeasureDock::build_dist_pannel() {
  ...
  for (auto &o : mode_rows->_dist_row_list) {
    ...
    connect(del_btn, &QPushButton::clicked, this,
            &MeasureDock::del_dist_measure);
    connect(s_btn, &QPushButton::clicked, this,
            &MeasureDock::popup_all_coursors);
...
void MeasureDock::build_edge_pannel() {
  ...
  for (auto &o : mode_rows->_edge_row_list) {
    ...
    connect(del_btn, &QPushButton::clicked, this,
            &MeasureDock::del_edge_measure);
    connect(s_btn, &QPushButton::clicked, this,
            &MeasureDock::popup_all_coursors);
    connect(e_btn, &QPushButton::clicked, this,
            &MeasureDock::popup_all_coursors);
    connect(ch_cmb, QOverload<int>::of(&QComboBox::currentIndexChanged), this,
            &MeasureDock::on_edge_channel_selected);
  }
...
void MeasureDock::popup_all_coursors() {
  ...
  cursor_dlg.setWindowFlags(
      Qt::FramelessWindowHint | Qt::Popup | Qt::WindowSystemMenuHint |
  ...
  for (auto i = cursor_list.begin(); i != cursor_list.end(); i++) {
    ...
    connect(cursor_btn, &QPushButton::clicked, this,
            &MeasureDock::set_sel_cursor);
    index++;
  }

  QRect sel_btn_rect = _sel_btn->geometry();
  sel_btn_rect.moveTopLeft(
      _sel_btn->parentWidget()->mapToGlobal(sel_btn_rect.topLeft()));
...
void MeasureDock::update_dist() {
  ...
  for (auto &inf : mode_rows->_dist_row_list) {
    ...
    if (inf.cursor1 != -1 && inf.cursor2 != -1) {
      ...
      QString delta_text =
          _view->get_cm_delta(inf.cursor1 - 1, inf.cursor2 - 1) + "/" +
...
void MeasureDock::set_cursor_btn_color(QPushButton *btn) {
  ...
  QColor cursor_color =
      isCursor ? view::Ruler::GetColorByCursorOrder(start) : bkColor;
  set_cursor_btn_color(btn, cursor_color, bkColor, isCursor);
}

void MeasureDock::set_cursor_btn_color(QPushButton *btn, QColor cursorColor,
                                       QColor bkColor, bool isCursor) {
  ...
  QString normal = "{background-color:" + cursorColor.name() +
                   ...
                   ";}";
  QString hover = "{background-color:" + hoverColor + "; color:" + textColor +
                  "; border-width:" + border_width + ";}";
  QString style = "QPushButton:hover" + hover + "QPushButton" + normal;
...
void MeasureDock::adjusLabelSize() { this->adjust_form_size(this); }
...
void MeasureDock::build_cursor_pannel() {
  ...
  for (; it != cursor_list.end(); it++) {
    ...
    del_btn->setIcon(IconCache::Instance().icon(iconPath + "/del.svg"));
    del_btn->setCheckable(true);
    QPushButton *cursor_pushButton =
        new QPushButton(QString::number(index), _widget);
...
void MeasureDock::UpdateLanguage() { retranslateUi(); }

void MeasureDock::UpdateTheme() { reStyle(); }
...
void MeasureDock::adjust_form_size(QWidget *wid) {
  ...
  for (auto o : labels) {
    if (o->wordWrap())
      continue; // Never force fixed width on wrapped labels
