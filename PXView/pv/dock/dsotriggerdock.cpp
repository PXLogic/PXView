...
DsoTriggerDock::DsoTriggerDock(QWidget *parent, SigSession *session)
  ...
  _holdoff_comboBox->setObjectName("dock_content");
  // tr
  _holdoff_comboBox->addItem(L_S(STR_PAGE_DLG, S_ID(IDS_DLG_S), "S"),
                             QVariant::fromValue(1000000000));
  _holdoff_comboBox->addItem(L_S(STR_PAGE_DLG, S_ID(IDS_DLG_MS), "mS"),
                             QVariant::fromValue(1000000));
  _holdoff_comboBox->addItem(L_S(STR_PAGE_DLG, S_ID(IDS_DLG_US), "uS"),
                             QVariant::fromValue(1000));
  ...
  _channel_comboBox->setObjectName("dock_content");
...
DsoTriggerDock::~DsoTriggerDock() { REMOVE_UI(this); }

void DsoTriggerDock::retranslateUi() {
  _position_label->setText(
      L_S(STR_PAGE_DLG, S_ID(IDS_DLG_TRIGGER_POSITION), "Trigger Position: "));
  _holdoff_label->setText(
      L_S(STR_PAGE_DLG, S_ID(IDS_DLG_HOLD_OFF_TIME), "Hold Off Time: "));
  _margin_label->setText(L_S(STR_PAGE_DLG, S_ID(IDS_DLG_NOISE_SENSITIVITY),
                             "Noise Sensitivity: "));
  _tSource_label->setText(
      L_S(STR_PAGE_DLG, S_ID(IDS_DLG_TRIGGER_SOURCES), "Trigger Sources: "));
  _tType_label->setText(
      L_S(STR_PAGE_DLG, S_ID(IDS_DLG_TRIGGER_TYPES), "Trigger Types: "));
  _rising_radioButton->setText(
      L_S(STR_PAGE_DLG, S_ID(IDS_DLG_RISING_EDGE), "Rising Edge"));
  _falling_radioButton->setText(
      L_S(STR_PAGE_DLG, S_ID(IDS_DLG_FALLING_EDGE), "Falling Edge"));

  _auto_radioButton->setText(L_S(STR_PAGE_DLG, S_ID(IDS_DLG_AUTO), "Auto"));
  _ch0_radioButton->setText(
      L_S(STR_PAGE_DLG, S_ID(IDS_DLG_CHANNEL_0), "Channel 0"));
  _ch1_radioButton->setText(
      L_S(STR_PAGE_DLG, S_ID(IDS_DLG_CHANNEL_1), "Channel 1"));
  _ch0a1_radioButton->setText(
      L_S(STR_PAGE_DLG, S_ID(IDS_DLG_CHANNEL_0_AND_1), "Channel 0 && 1"));
  _ch0o1_radioButton->setText(
      L_S(STR_PAGE_DLG, S_ID(IDS_DLG_CHANNEL_0_OR_1), "Channel 0 | 1"));
}

void DsoTriggerDock::reStyle() {}

void DsoTriggerDock::paintEvent(QPaintEvent *e) { (void)e; }
...
void DsoTriggerDock::pos_changed(int pos) {
  ...
  ret = _signals->device()->set_config_double(SR_CONF_HORIZ_TRIGGERPOS, static_cast<double>(pos));
  if (!ret) {
...
void DsoTriggerDock::source_changed() {
  if (check_trig_channel() == false) {
    ...
    QString msg(L_S(STR_PAGE_MSG, S_ID(IDS_MSG_DISABLED_CHANNEL_TRIG),
                    "Disabled channels cannot be used for triggering!"));
    MsgBox::Show(msg);
  }
...
void DsoTriggerDock::update_view() {
  ...
  if (_signals->device()->get_config_double(SR_CONF_HORIZ_TRIGGERPOS, dpos)) {
    _position_slider->setValue(static_cast<int>(dpos));
  }
...
void DsoTriggerDock::UpdateTheme() { reStyle(); }
