...
/* Forward declaration - the driver_info struct is defined at the bottom */
static struct sr_dev_driver *saleae_logic16_drv_ptr;
...
static GSList *scan(struct sr_dev_driver *di, GSList *options)
{
	...
	return std_scan_complete_compat(saleae_logic16_drv_ptr, devices);
}
	...
	case SR_CONF_VOLTAGE_THRESHOLD:
		...
		for (i = 0; i < ARRAY_SIZE(thresholds); i++) {
			if (devc->selected_voltage_range != thresholds_ranges[i].range)
				continue;
			*data = std_gvar_min_max_step(thresholds[i][0], thresholds[i][1], 0.0);
			return SR_OK;
		}
		return SR_ERR;
	default:
	...
	const struct sr_dev_inst *sdi, const struct sr_channel_group *cg)
{
	...
	case SR_CONF_VOLTAGE_THRESHOLD:
		...
		for (i = 0; i < ARRAY_SIZE(thresholds); i++) {
			if (fabs(low - thresholds[i][0]) < 0.01 &&
			    fabs(high - thresholds[i][1]) < 0.01) {
				devc->selected_voltage_range = thresholds_ranges[i].range;
				break;
			}
		}
...
static int receive_data(int fd, int revents, const struct sr_dev_inst *cb_data)
{
	struct timeval tv;
	struct dev_context *devc;
	struct drv_context *drvc;
...
static int dev_acquisition_start(const struct sr_dev_inst *sdi)
{
	...
	usb_source_add(devc->ctx, timeout, receive_data, sdi);
...
/* PXView compat driver pointer */
static int saleae_logic16_compat_init(struct sr_context *sr_ctx) {
	saleae_logic16_drv_ptr = &saleae_logic16_driver_info;
	return std_init(saleae_logic16_drv_ptr, sr_ctx);
}
static int saleae_logic16_compat_cleanup(void) {
	return std_cleanup(saleae_logic16_drv_ptr);
}
static GSList *saleae_logic16_compat_scan(GSList *options) {
	return scan(saleae_logic16_drv_ptr, options);
}
static int saleae_logic16_compat_config_get(int id, GVariant **data,
    const struct sr_dev_inst *sdi, const struct sr_channel *ch,
    const struct sr_channel_group *cg) {
    (void)ch;
    return config_get((uint32_t)id, data, sdi, cg);
}
static int saleae_logic16_compat_config_set(int id, GVariant *data,
    struct sr_dev_inst *sdi, struct sr_channel *ch,
    struct sr_channel_group *cg) {
    (void)ch;
    return config_set((uint32_t)id, data, sdi, cg);
}
static int saleae_logic16_compat_config_list(int info_id, GVariant **data,
    const struct sr_dev_inst *sdi, const struct sr_channel_group *cg) {
    return config_list((uint32_t)info_id, data, sdi, cg);
}
static int saleae_logic16_compat_acquisition_start(struct sr_dev_inst *sdi, void *cb_data) {
    (void)cb_data;
    return dev_acquisition_start(sdi);
}
static int saleae_logic16_compat_acquisition_stop(const struct sr_dev_inst *sdi, void *cb_data) {
    (void)cb_data;
...
struct sr_dev_driver saleae_logic16_driver_info = {
    ...
    .init = saleae_logic16_compat_init,
    .cleanup = saleae_logic16_compat_cleanup,
    .scan = saleae_logic16_compat_scan,
    .dev_mode_list = compat_dev_mode_list_default,
    .config_get = saleae_logic16_compat_config_get,
    .config_set = saleae_logic16_compat_config_set,
    .config_list = saleae_logic16_compat_config_list,
    .dev_open = dev_open,
    .dev_close = dev_close,
    .dev_destroy = compat_dev_destroy_default,
    .dev_status_get = compat_dev_status_get_default,
    .dev_acquisition_start = saleae_logic16_compat_acquisition_start,
    .dev_acquisition_stop = saleae_logic16_compat_acquisition_stop,
    .priv = NULL,
};
