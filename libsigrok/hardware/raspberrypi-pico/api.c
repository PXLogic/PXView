...
/* PXView's sr_receive_data_callback_t takes (int fd, int revents, const struct sr_dev_inst *sdi)
 * but the original raspberrypi_pico_receive takes (int fd, int revents, void *cb_data).
 * This wrapper adapts the PXView callback signature to the original function. */
static int raspberrypi_pico_receive_wrapper(int fd, int revents, const struct sr_dev_inst *sdi)
{
	return raspberrypi_pico_receive(fd, revents, (void *)sdi);
}
...
static struct sr_dev_driver *raspberrypi_pico_drv_ptr;
...
static GSList *scan(struct sr_dev_driver *di, GSList * options)
{
	...
	/* Use compat_sr_dev_inst_new instead of g_malloc0 */
	sdi = compat_sr_dev_inst_new(SR_INST_SERIAL, SR_ST_INACTIVE,
		"Raspberry Pi", "PICO", "00");
	sdi->conn = serial;
...
static int dev_acquisition_start(const struct sr_dev_inst *sdi)
{
	...
	struct dev_context *devc;
	struct sr_channel *ch;
	/* PXView compat: sr_session_trigger_get() always returns NULL,
	 * so skip the entire trigger code path that references
	 * struct sr_trigger/stage/match which are not defined here. */
#if 0 /* trigger support disabled for PXView compat */
	...
	len = serial_read_blocking(serial, devc->buffer, devc->serial_buffer_size,
		serial_timeout(serial, 115200, 4));
...
#if 0 /* trigger support disabled for PXView compat - sr_session_trigger_get returns NULL */
	if ((trigger = sr_session_trigger_get(NULL))) {
		...
		sr_session_source_add(-1, G_IO_IN, 200,
			raspberrypi_pico_receive_wrapper, sdi);
...
#endif /* trigger support disabled */
	{
		devc->trigger_fired = TRUE;
...
static int raspberrypi_pico_compat_init(struct sr_context *sr_ctx)
{
	raspberrypi_pico_drv_ptr = &raspberrypi_pico_driver_info;
	return std_init(raspberrypi_pico_drv_ptr, sr_ctx);
}

/* Wrapper: PXView cleanup(void) -> standard cleanup(driver) */
static int raspberrypi_pico_compat_cleanup(void)
{
	std_dev_clear(raspberrypi_pico_drv_ptr);
	return std_cleanup(raspberrypi_pico_drv_ptr);
}

/* Wrapper: PXView scan(options) -> standard scan(driver, options) */
static GSList *raspberrypi_pico_compat_scan(GSList *options)
{
	return scan(raspberrypi_pico_drv_ptr, options);
}

/* Wrapper: PXView dev_list(void) -> standard dev_list(driver) */
static GSList *raspberrypi_pico_compat_dev_list(void)
{
	return std_dev_list(raspberrypi_pico_drv_ptr);
}

/* Wrapper: PXView dev_clear(void) -> standard dev_clear(driver) */
static int raspberrypi_pico_compat_dev_clear(void)
{
	return std_dev_clear(raspberrypi_pico_drv_ptr);
}

/* Wrapper: PXView config_get(id,data,sdi,ch,cg) -> standard(key,data,sdi,cg) */
static int raspberrypi_pico_compat_config_get(int id, GVariant **data,
...
static int raspberrypi_pico_compat_config_set(int id, GVariant *data,
...
static int raspberrypi_pico_compat_config_list(int info_id, GVariant **data,
	const struct sr_dev_inst *sdi, const struct sr_channel_group *cg)
{
	return config_list((uint32_t)info_id, data, sdi, cg);
}

/* Wrapper: PXView dev_open(sdi) -> standard std_serial_dev_open(sdi) */
static int raspberrypi_pico_compat_dev_open(struct sr_dev_inst *sdi)
{
	return std_serial_dev_open(sdi);
}

/* Wrapper: PXView dev_close(sdi) -> standard std_serial_dev_close(sdi) */
static int raspberrypi_pico_compat_dev_close(struct sr_dev_inst *sdi)
{
	return std_serial_dev_close(sdi);
}

/* Wrapper: PXView acquisition_start(sdi, cb_data) -> standard(sdi) */
static int raspberrypi_pico_compat_acquisition_start(struct sr_dev_inst *sdi, void *cb_data)
{
	(void)cb_data;
...
static int raspberrypi_pico_compat_acquisition_stop(const struct sr_dev_inst *sdi, void *cb_data)
{
	(void)cb_data;
...
struct sr_dev_driver raspberrypi_pico_driver_info = {
	...
	.init = raspberrypi_pico_compat_init,
	.cleanup = raspberrypi_pico_compat_cleanup,
	.scan = raspberrypi_pico_compat_scan,
	.dev_mode_list = compat_dev_mode_list_default,
	.config_get = raspberrypi_pico_compat_config_get,
	.config_set = raspberrypi_pico_compat_config_set,
	.config_list = raspberrypi_pico_compat_config_list,
	.dev_open = raspberrypi_pico_compat_dev_open,
	.dev_close = raspberrypi_pico_compat_dev_close,
	.dev_destroy = compat_dev_destroy_default,
	.dev_status_get = compat_dev_status_get_default,
	.dev_acquisition_start = raspberrypi_pico_compat_acquisition_start,
	.dev_acquisition_stop = raspberrypi_pico_compat_acquisition_stop,
	.priv = NULL,
};
