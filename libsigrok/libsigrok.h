...
enum {
  ...
  SR_ERR_DEVICE_IS_EXCLUSIVE =
      11, /**< The device is exclusive by other process.*/
  SR_ERR_DEVICE_FIRMWARE_VERSION_LOW =
      12,                          /**< The firmware version is too low.*/
...
struct sr_channel {
  ...
  gboolean enabled;
  char *name;
  /** Back-reference to owning device (compat). */
  struct sr_dev_inst *sdi;
  /** Private data for driver use (compat). */
  void *priv;
  char *trigger;
  uint8_t bits;
  uint64_t vdiv;
...
enum sr_config_option_id {
  ...
  SR_CONF_DISK_CACHE_ENABLE = 60011,
  SR_CONF_DISK_CACHE_PATH = 60012,
  SR_CONF_STREAM_MEM_BUFF = 60013,

    /* Compat: Standard sigrok keys not originally in PXView */
    SR_CONF_TRIGGER_MATCH = 40100,
    SR_CONF_SCAN_OPTIONS = 40200,
    SR_CONF_VOLTAGE_THRESHOLD = 40300,
    SR_CONF_EXTERNAL_CLOCK = 40400,
    SR_CONF_PROBE_NAMES = 20002,
    SR_CONF_NUM_LOGIC_CHANNELS = 40500,
    SR_CONF_NUM_ANALOG_CHANNELS = 40501,
...
SR_API int ds_trigger_stage_set_value(uint16_t stage, uint16_t probes,
                                      char *trigger0, char *trigger1);
SR_API int ds_trigger_stage_set_logic(uint16_t stage, uint16_t probes,
                                      unsigned char trigger_logic);
SR_API int ds_trigger_stage_set_inv(uint16_t stage, uint16_t probes,
                                    unsigned char trigger0_inv,
                                    unsigned char trigger1_inv);
SR_API int ds_trigger_stage_set_count(uint16_t stage, uint16_t probes,
...
// The current device detached, user need to call ds_get_device_list to get the
// list, and call ds_active_device() to switch to the current device.
// #define DS_EV_CURRENT_DEVICE_DETACH     2

// A inactive device detached.
// User can call ds_get_device_list() to get the new list, and update the list
// view.
 ...
 * out_list is null.
 */
SR_API int ds_get_device_list(struct ds_device_base_info **out_list,
                              int *out_count);
...
SR_API const struct sr_config_info *
ds_get_actived_device_config_info_by_name(const char *optname);
...
*/
SR_API int ds_dsl_option_value_to_code(int work_mode, int config_id,
                                       const char *value);

/*----------channel----------*/
SR_API int ds_enable_device_channel(const struct sr_channel *ch,
                                    gboolean enable);
