...
class Ds28ea00Generator:
    """1 channel: 1-Wire bus (owr).
    Generates 1-Wire waveform for DS28EA00 digital thermometer with PIO.
    The DS28EA00 decoder sits on top of the onewire_network stack."""
    ...
    def generate_testdata(self):
        # Transaction 1: Skip ROM + Convert Temperature (0x44)
        self._reset()
        self._write_byte(0xCC)  # Skip ROM command
        self._write_byte(0x44)  # Convert Temperature
        self._read_byte(0xAA)   # Conversion status byte

        # Transaction 2: Skip ROM + Read Scratchpad (0xBE)
        self._reset()
        self._write_byte(0xCC)  # Skip ROM
        self._write_byte(0xBE)  # Read Scratchpad
        # Read 9 scratchpad bytes (temp LSB, temp MSB, TH, TL, config, reserved x3, CRC)
        for b in [0x50, 0x00, 0xFF, 0x00, 0x7F, 0xFF, 0xFF, 0xFF, 0x00]:
            self._read_byte(b)

        # Transaction 3: Skip ROM + PIO Access Read (0xF5)
        self._reset()
        self._write_byte(0xCC)  # Skip ROM
        self._write_byte(0xF5)  # PIO Access Read
        self._read_byte(0x0F)   # PIO sample
