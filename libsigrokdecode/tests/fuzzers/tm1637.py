...
class Tm1637Generator:
    """TM1637 LED driver generator.
    TM1637 is a stack decoder (input: tmc), so this generator produces
    TMC protocol waveforms that the tmc_c decoder will decode and pass up.
    TMC Wire3 mode: STB falling = START, CLK rising samples DIO, STB rising = STOP.
    TMC Wire2 mode: CLK high + DIO falling = START, CLK rising samples DIO,
    ...
    def generate_testdata(self):
        ...
        if stb >= 0:
            ...
            # Second transaction: Display command 0x8F (display on, contrast 14/16)
            self.builder.set_level(stb, 0, half_period)
