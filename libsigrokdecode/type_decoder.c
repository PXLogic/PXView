    ...
    struct srd_proto_data* pdata)
{
    ...
    if (!PyLong_Check(py_tmp)) {
        srd_err("Protocol decoder %s submitted SRD_OUTPUT_BINARY list, "
                "but first element was not an integer.",
            di->decoder->name);
        goto err;
    }
    ...
    if (!PyBytes_Check(py_tmp)) {
        srd_err("Protocol decoder %s submitted SRD_OUTPUT_BINARY list, "
                "but second element was not bytes.",
            di->decoder->name);
        goto err;
    }
    ...
    const GSList* stack, const PyObject* obj)
{
    ...
    g_rw_lock_reader_lock(&sessions_rwlock);

    if (!sessions) {
        g_rw_lock_reader_unlock(&sessions_rwlock);
        return NULL;
    }
...
static PyObject* Decoder_has_channel(PyObject* self, PyObject* args)
{
    ...
    count = g_slist_length(di->decoder->channels) + g_slist_length(di->decoder->opt_channels);
    if (idx < 0 || idx >= count) {
