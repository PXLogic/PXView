...
# 给各个变量分配唯一ID（0 ~ 19）属于Ann类下
class Ann:
    RX_WARN, TX_WARN, RX_DATA, TX_DATA, RX_START, TX_START,  \
    RX_PARITY_OK, TX_PARITY_OK, RX_PARITY_ERR, TX_PARITY_ERR, RX_STOP, TX_STOP,  \
    RX_DATA_BIT, TX_DATA_BIT, RX_BREAK, TX_BREAK, RX_PACKET, TX_PACKET, \
    RX_SAMPLES, TX_SAMPLES, ATK_POINT, \
        = range(21)
...
class Decoder(srd.Decoder):
    ...
    options = (
        ...
        {'id': 'invert_rx', 'desc': 'Invert RX(反转信号)', 'default': 'no',
            'values': ('yes', 'no')},
        {'id': 'invert_tx', 'desc': 'Invert TX(反转信号)', 'default': 'no',
            'values': ('yes', 'no')},
        {'id': 'put_sample_points', 'desc': 'Put sample points(显示采样点)', 'default': 'no',
            'values': ('yes', 'no')},
        {'id': 'show_start_stop', 'desc': 'Show start bit and stop bit(显示起/停)', 'default': 'yes',
            'values': ('yes', 'no')},
        {'id': 'sample_point', 'desc': 'Sample point (采样率%)', 'default': 50},
        {'id': 'packet_idle_us', 'desc': 'Packet delimit by idle time, us(按空闲时间划分包)', 'default': -1},
        {'id': 'rx_packet_delim', 'desc': 'RX packet delimiter (decimal)(RX包分隔符)',
            'default': -1},
        {'id': 'tx_packet_delim', 'desc': 'TX packet delimiter (decimal)(TX包分隔符)',
            'default': -1},
        {'id': 'rx_packet_len', 'desc': 'RX packet length(RX包长度)', 'default': -1},
        {'id': 'tx_packet_len', 'desc': 'TX packet length(TX包长度)', 'default': -1},
    ...
    annotations = (
        ...
        ('rx-data-bit', 'RX data bit'),#12
        ('tx-data-bit', 'TX data bit'),
        ('rx-break', 'RX break'),#14
        ('tx-break', 'TX break'),
        ('rx-packet', 'RX packet'),#16
        ('tx-packet', 'TX packet'),
        ('rx-sample', 'RX sample'),#18
        ('tx-sample', 'TX sample'),
    ...
    def start(self):
        ...
        self.show_start_stop = self.options['show_start_stop'] == 'yes'
    ...
    def get_start_bit(self, rxtx, signal):
        ...
        if startbit != 0:
            ...
            self.putgse(frame_ss, frame_es, [Ann.RX_WARN + rxtx, ['Start bit error', 'Start err', 'SE']])
    ...
    def handle_data(self, rxtx):
        ...
        self.putgse(ss_data, es_data, [Ann.RX_DATA + rxtx, [self.format_value(b)]])
    ...
    def init_format(self):
        ...
        if fmt == 'dec':
            ...
            if fmt == 'hex':        # 十六进制（hex）：每 4 位二进制对应 1 位十六进制，向上取整
                digits = (self.data_bits + 4 - 1) // 4
                fmtchar = "X"
            elif fmt == 'oct':      # 八进制（oct）：每 3 位二进制对应 1 位八进制，向上取整
                digits = (self.data_bits + 3 - 1) // 3
                fmtchar = "o"
            elif fmt == 'bin':      # 二进制（bin）：位数等于数据位数本身
                digits = self.data_bits
                fmtchar = "b"
            else:
                fmtchar = None
            # 生成格式化字符串：如 "{:04X}"、"{:08b}" 等
    ...
    def get_stop_bits(self, rxtx, signal):
        ...
        if stopbit_error:
            es = self.samplenum
            # 输出错误标注（协议层、全局注解、警告信息）
            self.putpse(ss, es, ['INVALID STOPBIT', rxtx, signal])  # 协议层错误
            # self.putgse(ss, es, [Ann.RX_WARN + rxtx, ['Frame error', 'Frame err', 'FE']])   # 全局警告
            # self.putgse(ss, es, [Ann.RX_STOP + rxtx, ['Stop bit error', 'Stop err', 'TE']]) # 停止位错误
            self.putgse(ss, es, [Ann.RX_WARN + rxtx, ['Stop bit error', 'Stop err', 'TE']]) # 停止位错误
            self.frame_valid[rxtx] = False  #标记当前帧无效
    ...
    def handle_frame(self, rxtx, ss, es):
        ...
        self.putbinse(ss, es, [Bin.RX + rxtx, bdata])  # rx/tx
        self.putbinse(ss, es, [Bin.RXTX, bdata])       # rxtx

        # 仅无错误数据输出到 rx-ok/tx-ok/rxtx-ok
        if self.frame_valid[rxtx]:
            self.putbinse(ss, es, [Bin.RX_OK + rxtx, bdata])  # rx-ok/tx-ok
            self.putbinse(ss, es, [Bin.RXTX_OK, bdata])        # rxtx-ok
        ...
        while True:
            ...
            for rxtx in (RX, TX):
                if has_pin[rxtx]:
                    ...
                    if cond_idle_idx[rxtx] is not None and (self.matched & (0b1 << cond_idle_idx[rxtx])):
                        self.inspect_idle(rxtx, signal[rxtx], inv[rxtx])