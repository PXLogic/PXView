...
class Decoder(srd.Decoder):
    ...
    def handle_bits(self):
        ...
        # if fmt == 'hex':
        #     s = ['DATA-VALUE: %d / 0x%x' % (v, v), 'D-VAL: %2X' % v, 'V', 'V', 'V']
        # elif fmt == 'dec':
        #     s = ['DATA-VALUE: %d / 0x%x' % (v, v), 'D-VAL: %d' % v, 'V', 'V', 'V']
        # elif fmt == 'oct':
        #     s = ['DATA-VALUE: %d / 0x%x' % (v, v), 'D-VAL: %03o' % v, 'V', 'V', 'V']
        # elif fmt == 'bin':
        #     s = ['DATA-VALUE: %d / 0x%x' % (v, v), 'D-VAL: %08b' % v, 'V', 'V', 'V']
        format_map = {
            'hex': ('%2X', 'hex'),
            'dec': ('%d', 'dec'),
            'oct': ('%03o', 'oct'),
        ...
        format_str, fmt_name = format_map.get(fmt, ('%d', 'dec'))  # 默认使用十进制

        s = [
            f'DATA-VALUE: {v} / 0x{v:x}',  # 始终显示十进制和十六进制
            f'D-VAL-{fmt_name}: {format_str % v}',
            'V', 
            'V', 
            'V'
        ]
