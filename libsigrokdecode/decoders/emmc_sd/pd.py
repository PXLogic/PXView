...
class Decoder(srd.Decoder):
    ...
    annotations = \
        ...
        ('field-crc', 'CRC'),
        ('field-end', 'End bit'),
        ('decoded-bit', 'Decoded bits'),
        ('decoded-field', 'Decoded fields'),
    )
    annotation_rows = (
        ('raw-bits', 'Raw bits', (64,)),
        ('decoded-bits', 'Decoded bits', (71,)),
        ('decoded-fields', 'Decoded fields', (72,)),
        ('fields', 'Fields', tuple(range(65, 72))),
        ('cmds', 'Commands', tuple(range(0, 64))),
    )
    ...
    def putbit(self, b, data):
        self.put(self.token[b][0], self.token[b][1], self.out_ann, [71, data])
    ...
    def handle_common_token_fields(self):
        ...
        for bit in range(len(self.token)):
            self.putf(bit, bit, [71, ['%d' % s[bit][2]]])

        # CMD[47:47]: Start bit (always 0)
        self.putf(0, 0, [65, ['Start bit', 'Start', 'S']])

        # CMD[46:46]: Transmission bit (1 == host)
        t = 'host' if s[1][2] == 1 else 'card'
        self.putf(1, 1, [66, ['Transmission: ' + t, 'T: ' + t, 'T']])
        ...
        self.putf(2, 7, [67, ['Command: ' + c, 'Cmd: ' + c,
                               'CMD%d' % self.cmd, 'Cmd', 'C']])

        # CMD[39:08]: Argument
        self.arg = int('0b' + ''.join([str(s[i][2]) for i in range(8, 40)]), 2)
        self.putf(8, 39, [68, ['Argument: 0x%08x' % self.arg, 'Arg', 'A']])

        # CMD[07:01]: CRC7
        self.crc = int('0b' + ''.join([str(s[i][2]) for i in range(40, 47)]), 2)
        self.putf(40, 46, [69, ['CRC: 0x%x' % self.crc, 'CRC', 'C']])

        # CMD[00:00]: End bit (always 1)
        self.putf(47, 47, [70, ['End bit', 'End', 'E']])
    ...
    def handle_cmd0(self):
        ...
        self.puta(0, 31, [72, ['IDLE_STATE', 'IDLE', 'ID', 'I']])
    ...
    def handle_cmd4(self):
        ...
        self.puta(16, 31, [72, ['Set DSR', 'SDSR']])
        self.puta(0, 15, [72, ['Stuff bits', 'Stuff', 'SB', 'S']])
    ...
    def handle_cmd5(self):
        ...
        self.puta(16, 31, [72, ['Set DSR', 'SDSR']])
        self.puta(15, 15, [72, ['Sleep/Awake', 'S/A']])
        self.puta(0, 14, [72, ['Stuff bits', 'Stuff', 'SB', 'S']])
        self.putc(5, 'Sleep/Awake')
        self.token, self.state = [], 'GET RESPONSE R1b'

    def handle_cmd6(self):
        # CMD6 (SWITCH_FUNC) -> R1b
        self.puta(26, 31, [72, ['Set to 0', 'Set 0', 'S0', 'Z']])
        self.puta(24, 25, [72, ['Access', 'A']])
        self.puta(16, 23, [72, ['Index', 'Id']])
        self.puta(8, 15, [72, ['Value', 'Val', 'V']])
        self.puta(3, 7, [72, ['Set to 0', 'Set 0', 'S0', 'Z']])
        self.puta(0, 2, [72, ['CMD Set', 'CMD S']])
    ...
    def handle_cmd12(self):
        ...
        self.puta(16, 31, [72, ['RCA', 'R']])
        self.puta(1, 15, [72, ['Stuff bits', 'Stuff', 'SB', 'S']])
        self.puta(0, 0, [72, ['HPI']])
    ...
    def handle_cmd13(self):
        # CMD13 (SEND_STATUS) -> R1
        self.puta(16, 31, [72, ['RCA', 'R']])
        self.puta(15, 15, [72, ['SQS']])
        self.puta(1, 14, [72, ['Stuff bits', 'Stuff', 'SB', 'S']])
        self.puta(0, 0, [72, ['HPI']])
    ...
    def handle_cmd23(self):
        ...
        self.puta(30, 30, [72, ['Packed']])
        if self.token[30] == 1:
            self.puta(31, 31, [72, ['Set to 0', 'Set 0', 'S0', 'Z']])
            self.puta(16, 29, [72, ['Set to 0', 'Set 0', 'S0', 'Z']])
        else:
            self.puta(31, 31, [72, ['Reliable Write', 'RLB W']])
            self.puta(25, 28, [72, ['Context ID', 'CntxtID']])
            self.puta(24, 24, [72, ['Forced Programming', 'Forced Prog', 'FP']])
            self.puta(16, 23, [72, ['Set to 0', 'Set 0', 'S0', 'Z']])

        self.puta(0, 15, [72, ['Number of Blocks', 'NUM BLK', 'NoB']])
        self.putc(23, 'Defines the number of blocks')
        self.token, self.state = [], 'GET RESPONSE R1'

    def handle_cmd24(self):
        # CMD24 (WRITE_BLOCK) -> R1
        self.puta(0, 31, [72, ['Data Address', 'DAT ADDR', 'DA']])
    ...
    def handle_cmd38(self):
        ...
        self.puta(31, 31, [72, ['Secure Request', 'Sec Req', 'SR']])
        self.puta(16, 30, [72, ['Set to 0', 'Set 0', 'S0', 'Z']])
        self.puta(15, 15, [72, ['Force Garbage Collect', 'F Garb Clct', 'FGC']])
        self.puta(2, 14, [72, ['Set to 0', 'Set 0', 'S0', 'Z']])
        self.puta(1, 1, [72, ['Discard Enable', 'DISG EN', 'DE']])
        self.puta(0, 0, [72, ['TRIM Enable', 'TRIM EN', 'TE']])
    ...
    def handle_cmd39(self):
        # CMD39 (FAST_IO) -> R4
        self.puta(16, 31, [72, ['RCA', 'R']])
        self.puta(15, 15, [72, ['Write Flag', 'W Flag', 'WF', 'W']])
        self.puta(8, 14, [72, ['Register Addr', 'REGADD', 'RA']])
        self.puta(0, 7, [72, ['Register Data', 'REGDAT', 'DAT', 'D']])
    ...
    def handle_cmd44(self):
        ...
        self.puta(31, 31, [72, ['Reliable Write Request', 'RWR']])
        self.puta(30, 30, [72, ['Data Direction(1:R/0:W)', 'DD']])
        self.puta(29, 29, [72, ['Tag request', 'Tag Req', 'TR']])
        self.puta(25, 28, [72, ['Context ID', 'CntxtID']])
        self.puta(24, 24, [72, ['Forced Programming', 'F Prog', 'FP']])
        self.puta(23, 23, [72, ['Priority(0:Simple/1:High)', 'Prio']])
        self.puta(21, 22, [72, ['Reserved', 'RSVD']])
        self.puta(16, 20, [72, ['Task ID', 'TID']])
        self.puta(0, 15, [72, ['Block Numbers', 'Blk NUM']])
    ...
    def handle_cmd45(self):
        ...
        self.puta(0, 31, [72, ['Start of block address', 'Start BLK ADDR', 'SBA', 'SA']])
        self.putc(45, 'Defines the block address of queued task')
        self.token, self.state = [], 'GET RESPONSE R1'

    def handle_cmd46(self):
        # CMD46 (EXECUTE_READ_TASK) -> R1
        self.puta(21, 31, [72, ['Reserved', 'RSVD']])
        self.puta(16, 20, [72, ['Task ID', 'TID']])
        self.puta(0, 15, [72, ['Reserved', 'RSVD']])
    ...
    def handle_cmd48(self):
        ...
        self.puta(21, 31, [72, ['Reserved', 'RSVD']])
        self.puta(16, 20, [72, ['Task ID', 'TID']])
        self.puta(4, 15, [72, ['Reserved', 'RSVD']])
        self.puta(0, 3, [72, ['TM Op-code', 'TMOP']])
    ...
    def handle_cmd53(self):
        ...
        self.puta(16, 31, [72, ['Security Protolcol Specific', 'Sec P Spec', 'SPS']])
        self.puta(8, 15, [72, ['Security Protolcol', 'Sec P', 'SP']])
        self.puta(0, 7, [72, ['Reserved', 'RSVD']])
    ...
    def handle_cmd56(self):
        # CMD55 (GEN_CMD) -> R1
        self.puta(1, 31, [72, ['Stuff bits', 'Stuff', 'SB', 'S']])
        self.puta(0, 0, [72, ['RD/WR1', 'Stuff', 'SB', 'S']])
    ...
    def handle_response_r1(self, cmd):
        ...
        self.handle_common_token_fields()
        self.putr('R1')
        self.puta(0, 31, [72, ['Card status', 'Status', 'S']])
        for i in range(32):
    ...
    def handle_response_r2(self, cmd):
        ...
        # Annotations for each individual bit.
        for bit in range(len(self.token)):
            self.putf(bit, bit, [71, ['%d' % self.token[bit][2]]])
        self.putf(0, 0, [65, ['Start bit', 'Start', 'S']])
        t = 'host' if self.token[1][2] == 1 else 'card'
        self.putf(1, 1, [66, ['Transmission: ' + t, 'T: ' + t, 'T']])
        self.putf(2, 7, [67, ['Check Bits', 'CHECK', 'C']])
        self.putf(8, 134, [68, ['Argument', 'Arg', 'A']])
        self.putf(135, 135, [70, ['End bit', 'End', 'E']])
        self.putf(8, 134, [72, ['CID/CSD register', 'CID/CSD', 'C']])
    ...
    def handle_response_r3(self, cmd):
        ...
        self.putf(1, 1, [66, ['Transmission: ' + t, 'T: ' + t, 'T']])
        self.putf(2, 7, [67, ['Check bits', 'CHECK', 'C']])
        self.putf(8, 39, [68, ['Argument', 'Arg', 'A']])
        self.putf(40, 46, [69, ['Check bits', 'CHECK', 'C']])
        self.putf(47, 47, [70, ['End bit', 'End', 'E']])
        self.puta(0, 31, [72, ['OCR register', 'OCR reg', 'OCR', 'O']])
        self.token, self.state = [], 'GET COMMAND TOKEN'
    ...
    def handle_response_r4(self, cmd):
        ...
        if not self.get_token_bits(cmd, 39):
            return
        self.handle_common_token_fields()
        self.puta(0, 15, [72, ['Card status bits', 'Status', 'S']])
        self.puta(16, 31, [72, ['Relative card address', 'RCA', 'R']])
        self.putr('R4')
        self.token, self.state = [], 'GET COMMAND TOKEN'
    ...
    def handle_response_r5(self, cmd):
        ...
        # Arg[31:16]: RCA
        self.puta(16, 31, [72, ['RCA']])

        # Arg[15:00]: Echo-back of check pattern
        self.puta(0, 15, [72, ['Not Defined/IRQ data', 'NDEF']])
